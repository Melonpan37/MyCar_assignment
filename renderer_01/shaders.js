...
//IGNORE
lightShader = function (gl, nlamps) {
  var vertexShaderSource = `
    uniform   mat4 uModelViewMatrix;               
    uniform   mat4 uProjectionMatrix;              
    attribute vec3 aPosition;         
  ...
  var fragmentShaderSource = `
    ...
    {               
      ...
      //normale interpolata in view space
      vec3 N = normalize(vViewSpaceNormal);
      //direzione luce direzionale
      vec3 L = normalize(-uLightDirection.xyz);
      
      //contributo luce direzionale
      float NdotL = max(0.2, dot(N, L)); //la luce ambientale è data da 0.2                       
      vec3 lambert = (uColor.xyz * uLightColor) * NdotL;     
      
      for(int i = 0; i < spotlightsNumber; i++){
        //vettore sorgente spotlight -> posizione fragment        
        vec3 spotlightToSurface = normalize(vec3(uSpotlightsPosition[i]) - vViewSpaceFragmentPosition);
        //coseno dell'angolo fra vettore centrale al cono della luce spotlight e posizione del fragment
        float cosAngle = dot(spotlightToSurface, -uSpotlightsDirection.xyz);
        if(0.5 < cosAngle && cosAngle < 1.0){ //controlla se siamo all'interno del cono della spotlight
          L = spotlightToSurface;
          NdotL = max(0.0, dot(N, L)); //contributo spotlight
          float cosFactor = 8.0*(cosAngle-0.5)*(cosAngle-0.5); //fattore per attenuare la luce lungo i bordi del cono
          lambert += vec3(uColor)*NdotL*vec3(uSpotlightsColor)*cosFactor; //componente diffusa
        }
